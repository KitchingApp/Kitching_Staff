...
fun LoginScreen(
    ...
    val checkAndSaveUserState by loginViewModel.checkAndSaveUserResult.collectAsStateWithLifecycle()
    val saveUserIdState by loginViewModel.saveUserIdResult.collectAsStateWithLifecycle()

    if(kakaoLoginState is AppResult.Initial) {
        Column(
            modifier = Modifier
                .fillMaxSize(),
                        ...
                        Row(
                            ...
                            TextButton(
                                ...
                                onClick = {
                                    ...
                                    loginViewModel.performKakaoLogin(navController.context)
                                }
                            ) {
                                Text(
    ...
    if(kakaoLoginState is AppResult.Loading || checkAndSaveUserState is AppResult.Loading || saveUserIdState is AppResult.Loading) {
        ProgressIndicatorScreen()
    }
    if(kakaoLoginState is AppResult.Success && checkAndSaveUserState is AppResult.Success && saveUserIdState is AppResult.Success) {
        navController.navigate(ScreenRouteDef.TeamSelect.routeName)
    }
    if(kakaoLoginState is AppResult.Failure || checkAndSaveUserState is AppResult.Failure || saveUserIdState is AppResult.Failure) {
        // 재로그인 시도 창 띄우기
    }
}